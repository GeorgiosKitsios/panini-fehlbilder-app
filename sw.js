const CACHE='panini-fehlbilder-v12';
...
self.addEventListener('activate',event=>{
  event.waitUntil((async()=>{
    ...
    for(const client of clients){
      ...
      if(url.origin===self.location.origin){url.searchParams.set('appv','12');client.navigate(url.href);}
    }
  })());
});
...
async function appShell(request){
  ...
  html=html.replace('</body>','<script src="./list-groups.js?v=12"></script><script src="./local-ai.js?v=12"></script></body>');
